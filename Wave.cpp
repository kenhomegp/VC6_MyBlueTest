...
BOOL CWave::Open(SHORT channels, DWORD sampleRate, SHORT bitsPerSample)
{
	...
	wfex.cbSize = 0;
	if (waveOutOpen(&m_hWaveout, WAVE_MAPPER, &wfex, (unsigned long)&CWave::WaveOut_Proc, (unsigned long)this, CALLBACK_FUNCTION) != MMSYSERR_NOERROR)
...
BOOL CWave::WaveOut_Proc(HWAVEOUT hwi, UINT uMsg, DWORD dwInstance, DWORD dwParam1, DWORD dwParam2)
{
	...
	// Get current object
	CWave* pWave = (CWave*)((unsigned long *)&dwInstance);
