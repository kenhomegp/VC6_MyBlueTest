...
void CClockST::Refresh()
{
    ...
    int sec = csTime.wHour * 3600 + csTime.wMinute * 60 + csTime.wSecond ; 
	sec = sec - Inisec;

	csTime.wMinute = sec / 60 ;
    csTime.wSecond = sec % 60 ;
...
DWORD CClockST::Start(int nPaneID, int nBigID, int nSmallID, BOOL bAlternateDateFormat)
{
    ...
    cw = AfxBeginThread( &thrClock, (LPVOID)&m_thrParam );
...
void CClockST::StartTimer()
{
	Inisec = 0;
	SetTime();
   	::GetLocalTime(&IniTime);
    Inisec = IniTime.wHour * 3600 + IniTime.wMinute * 60 + IniTime.wSecond;

	SetTimer(1 , 1000 , 0);
}

void CClockST::SetTime()
{
	// Set day
	m_idxDigit[6] = 0;
	m_idxDigit[7] = 0;
